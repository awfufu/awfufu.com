...
把 `--external-device nvme0n1p1` 改成 `sda1`。

**刷到 SD 卡：**

把 `--external-device nvme0n1p1` 改成 `mmcblk0p1`。
...
其中 `Host` 后面的 `nrc17` 是你自己定义的别名。这样以后直接执行：
...
如果目标设备不在当前局域网，而是需要先经过一台跳板机，再访问内网里的 Nano，可以这样写：
